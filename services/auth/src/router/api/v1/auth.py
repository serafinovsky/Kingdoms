...
from dependencies.token import get_access_payload, get_fingerprint, get_refresh_payload
...
async def authorize(provider: Provider) -> str:
...
async def token(
    ...
    try:
        ...
        logger.exception(f"Authentication error: {str(e)}", stack_info=True, exc_info=e)
        raise HTTPException(
    ...
    refresh_payload = RefreshPayload(sub=str(user.id), fgp=fingerprint)
    access_payload = AccessPayload(sub=str(user.id))
...
async def refresh(
    ...
    if ttl <= settings.min_refresh_ttl:
        await blacklist_repo.add(redis, refresh_payload)
        refresh_payload = RefreshPayload(sub=refresh_payload.sub, fgp=fingerprint)
...
async def validate(
    ...
    return ORJSONResponse(content={"ok": True}, headers={"X-User-Id": access_payload.sub})
